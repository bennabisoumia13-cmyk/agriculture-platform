...
// ============================================
// دوال التمويل - الاتصال بـ APIs
// ============================================
const FINANCE_API_BASE = 'http://localhost:8000/api';

async function sendFinanceDataToAPI(fileData) {
    const recensement_id = fileData.exploitantId;
    if (!recensement_id) return;
    
    const financeSources = [];
    if (fileData.financePropress) financeSources.push(1);
    if (fileData.financeCredit) financeSources.push(2);
    if (fileData.financeSoutien) financeSources.push(3);
    if (fileData.financeEmprunt) financeSources.push(4);
    
    for (const type_id of financeSources) {
        try {
            await fetch(`${FINANCE_API_BASE}/financements/`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ recensement_id, type_financement_id: type_id })
            });
        } catch(e) { console.error(e); }
    }
    
    if (fileData.financeCredit && fileData.typeCredit) {
        let creditTypeId = { 'كلاسيكي': 2, 'Ettahadi': 1, 'Rfig': 4, 'Leasing': 3 }[fileData.typeCredit];
        if (creditTypeId) {
            try {
                await fetch(`${FINANCE_API_BASE}/credits/`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ recensement_id, type_credit_id: creditTypeId })
                });
            } catch(e) { console.error(e); }
        }
    }
    
    if (fileData.financeSoutien && fileData.typeSoutien) {
        let soutienTypeId = { 'مالي': 1, 'عتاد': 2, 'محاصيل': 3 }[fileData.typeSoutien];
        if (soutienTypeId) {
            try {
                await fetch(`${FINANCE_API_BASE}/soutiens/`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ recensement_id, type_soutien_id: soutienTypeId })
                });
            } catch(e) { console.error(e); }
        }
    }
    
    if (fileData.assuranceAgricole === 'نعم') {
        const assuranceTypes = [];
        if (fileData.assuranceTerre) assuranceTypes.push(1);
        if (fileData.assuranceMaterial) assuranceTypes.push(5);
        if (fileData.assuranceMahassel) assuranceTypes.push(3);
        if (fileData.assuranceMawachi) assuranceTypes.push(6);
        
        for (const type_id of assuranceTypes) {
            try {
                await fetch(`${FINANCE_API_BASE}/assurances/`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ recensement_id, type_assurance_id: type_id, est_inscrit: true, compagnie: fileData.compagnieAssurance || '' })
                });
            } catch(e) { console.error(e); }
        }
    }
...
function saveSurveyFile() {
    ...
    localStorage.setItem("farmers", JSON.stringify(farmers));
        sendFinanceDataToAPI(surveyFile);
...
function updateSurveyFile() {
    ...
    if (index !== -1) {
        ...
        localStorage.setItem("farmers", JSON.stringify(farmers));
            sendFinanceDataToAPI(updatedFile);
